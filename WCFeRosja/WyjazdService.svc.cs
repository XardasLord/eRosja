...
namespace WCFeRosja
{
    ...
    public class WyjazdService : IWyjazdService
    {
        ...
        public WyjazdyEntity PrzetworzWyjazdDlaBazy(Wyjazd daneWyjazdu)
        {
            ...
            using (eRosjaEntities eRosja = new eRosjaEntities())
            {
                int IdPrzejscia = eRosja.przejscia.Where(p => p.nazwa.Equals(daneWyjazdu.Przejscie)).Select(p => p.id_przejscia).First();
                int IdStacji = eRosja.stacje_benzynowe.Where(s => s.nazwa.Equals(daneWyjazdu.Paliwo.Stacja) &&
                ...
                if (papierosy == true)
                {
                    wyjazd.id_sklepy_papierosy = eRosja.sklepy.Where(s => s.nazwa.Equals(daneWyjazdu.Papierosy.Sklep)
                                                                 && s.id_przejscia == IdPrzejscia)
                                                                 .Select(s => s.id_sklepy).First();

                    wyjazd.id_papierosy = eRosja.papierosy_ceny.Where(p => p.papierosy.nazwa.Equals(daneWyjazdu.Papierosy.Nazwa) &&
            ...
            using (eRosjaEntities eRosja = new eRosjaEntities())
            {
                ...
                for (int i = 0; i < listaWyjazdowEntity.Count; i++)
                {
                    ...
                    if (wyjazdEntity.id_sklepy_papierosy != null)
                    {
                        ...
                        wyjazd.Papierosy.Sklep = eRosja.sklepy.Where(s => s.id_sklepy == wyjazdEntity.id_sklepy_papierosy).Select(s => s.nazwa).First();
                        wyjazd.Papierosy.Nazwa = eRosja.papierosy.Where(p => p.id_papierosy == wyjazdEntity.id_papierosy).Select(p => p.nazwa).First();
